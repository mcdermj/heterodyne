...
- (void) addItem:(SFBViewSelectorBarItem *)item
{
	...
	for(NSUInteger itemIndex = 0; itemIndex < itemCount; ++itemIndex) {
		NSRect cellRect = [self frameRectForCellAtIndex:itemIndex];
		SFBViewSelectorBarItem *itemInfo = [_items objectAtIndex:itemIndex];
...
- (SFBViewSelectorBarItem *) selectedItem
{
...
- (BOOL) selectItem:(SFBViewSelectorBarItem *)item
{
	...
	for(SFBViewSelectorBarItem *currentItem in _items) {
		if(item == currentItem) {
...
@implementation SFBViewSelectorBar (Private)
