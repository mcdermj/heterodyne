...
-(void)put: (NSData *) _data {
	@synchronized(data) {
		if([self space] < [_data length]) {
			NSLog(@"%@ [OzyRingBuffer put]: space=%d, wanted=%d\n", name, [self space], (int) [_data length]);
			return;
		}
	...
	BUFFERS_RINGBUFFER_PUT((char *) [name cStringUsingEncoding:NSASCIIStringEncoding], entries, [data length] - entries);
...
-(NSData *)get: (int)_size {
	...
	@synchronized(data) {
		if(entries < _size) {
			...
			BUFFERS_RINGBUFFER_GET((char *) [name cStringUsingEncoding:NSASCIIStringEncoding], entries, [data length] - entries);
			return nil;
		}
