...
-(id)init {
	...
	if(self) {
		...
		scale = highWaterLevel - lowWaterLevel;		
...
-(void)drawInCGLContext:(CGLContextObj)glContext
	...
	for(i = 0, c = 0; i < SPECTRUM_BUFFER_SIZE; i++) {
		int intensity = (int) floorf(((smoothBuffer[i] - lowWaterLevel) / scale) * 20000.0);
		intensity = intensity > 19999 ? 19999 : intensity;
		intensity = intensity < 0 ? 0 : intensity;
		
		line[i] = colorGradientArray[intensity];
	}
