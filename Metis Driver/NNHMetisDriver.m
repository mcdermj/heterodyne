...
+(NSString *)name {
	return @"Heterodyne Metis Driver";
}
...
-(void)loadParams {
	...
	if(sampleRate != [[NSUserDefaults standardUserDefaults] integerForKey:@"MetisDriver.sampleRate"]) {
...
-(void)performDiscovery {
	...
	if(setsockopt(CFSocketGetNative(metisSocket), SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(timeout)) == -1) {
	...
	while(gotDiscovery == NO) {
		...
		bytesReceived = recvfrom(CFSocketGetNative(metisSocket), 
								 &reply, 
								 sizeof(reply), 
								 0, 
