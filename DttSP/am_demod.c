...
static void
pll (AMD am, DTTSPCOMPLEX sig)
{
 DTTSPCOMPLEX z = Cmplx ((REAL) cos (am->pll.phs), (REAL) sin (am->pll.phs));
	REAL diff;
...
newAMD (REAL samprate,
	...
	REAL f_hibound,
	REAL f_bandwid,
	int size, DTTSPCOMPLEX * ivec, DTTSPCOMPLEX * ovec, AMMode mode, char *tag)
{
