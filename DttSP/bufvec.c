...
{
	return  (DTTSPCOMPLEX *) safealloc(size, sizeof (DTTSPCOMPLEX) , tag);
}

void
delvec_COMPLEX (DTTSPCOMPLEX *vec)
{
	safefree((char *) vec);
}
...
{
 DTTSPCOMPLEX *p = fftwf_malloc(size*sizeof (DTTSPCOMPLEX) );
	if (!p)
	{
		if (tag && *tag)
...
void
delvec_COMPLEX_fftw (DTTSPCOMPLEX *vec)
{
	fftwf_free((char *) vec);
}
...
CXB
newCXB(int size, DTTSPCOMPLEX *base, char *tag) 
{
...
REAL
normalize_vec_COMPLEX (DTTSPCOMPLEX *z, int n, REAL scale)
{
	if (z && (n > 0)) 
	{
		int i;
...
void
dump_CX(FILE * fp, char *head, DTTSPCOMPLEX *ptr, int beg, int fin) 
{
	int i;
	FILE *iop = fp ? fp : stderr;
	if (head && *head)
