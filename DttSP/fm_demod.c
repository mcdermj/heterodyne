...
PRIVATE void
pll (FMD fm, DTTSPCOMPLEX sig)
{
 DTTSPCOMPLEX z = Cmplx ((REAL) cos (fm->pll.phs), (IMAG) sin (fm->pll.phs));
	REAL diff;
...
newFMD (REAL samprate,
		...
		REAL f_lobound,
		REAL f_hibound,
		REAL f_bandwid, int size, DTTSPCOMPLEX * ivec, DTTSPCOMPLEX * ovec, char *tag)
{
