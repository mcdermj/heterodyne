	...
	float *output_r, unsigned int nframes, int thread)
{
	...
	{
		semaphore_signal(top[i].sync.buf.sem);
	}
}
...
//		sem_post (&top[thread].sync.buf.sem);
		semaphore_signal(top[thread].sync.buf.sem);
...
process_samples_thread (unsigned int proc_thread)
{
	...
	double mult = ((double)tTBI.denom / (double)tTBI.numer);

	ttcpolicy.period = 350000 * mult;
	ttcpolicy.computation = 200000 * mult;
	ttcpolicy.constraint = 1000000 * mult;
	ttcpolicy.preemptible=0;

	if((ret = thread_policy_set(mach_thread_self(), THREAD_TIME_CONSTRAINT_POLICY, (thread_policy_t)&ttcpolicy, THREAD_TIME_CONSTRAINT_POLICY_COUNT)) != KERN_SUCCESS) {
		fprintf(stderr, "Setting Realtime Failed\n");
	} 

	while (top[proc_thread].running)
	{
		semaphore_wait(top[proc_thread].sync.buf.sem);
		while (gethold(proc_thread)) {
			semaphore_wait(top[proc_thread].sync.upd.sem);
			...
			semaphore_signal(top[proc_thread].sync.upd.sem);
			puthold (proc_thread);
		}
	}
}
...
setup_threading (unsigned int thread)
{
		...
		semaphore_create(mach_task_self(), (semaphore_t *) &top[thread].sync.upd.sem, SYNC_POLICY_FIFO, 0);
		semaphore_create(mach_task_self(), (semaphore_t *) &top[thread].sync.buf.sem, SYNC_POLICY_FIFO, 0);
		pthread_create((pthread_t *) &top[thread].pid, 0, (void *) process_samples_thread, thread);
...
setup_defaults (unsigned int thread)
{
  ...
  sprintf (loc[thread].path.rcfile, "%s%0d_%0u", RCBASE, top[thread].pid,thread);
  sprintf (loc[thread].path.parm, "%s%0d_%0u", PARMPATH, top[thread].pid,thread);
  sprintf (loc[thread].path.meter, "%s%0d_%0u", METERPATH, top[thread].pid,thread);
  sprintf (loc[thread].path.spec, "%s%0d_%0u", SPECPATH, top[thread].pid,thread);
  sprintf (loc[thread].path.wisdom, "%s%0d_%0u", WISDOMPATH, top[thread].pid,thread);
