...
blms_adapt (BLMS blms)
{
 ...
 // fputs("Inside\n",stderr),fflush(stderr);
  do {
      int j;
      memcpy (blms->delay_line, &blms->delay_line[128], sizeof  (DTTSPCOMPLEX)  * 128);	// do overlap move
      memcpy (&blms->delay_line[128], &CXBdata (blms->signal, sigidx), sizeof  (DTTSPCOMPLEX)  * 128);	// copy in new data
      ...
      memset (blms->y, 0, 128 * sizeof  (DTTSPCOMPLEX) );
      for (j = 128; j < 256; j++)
		  blms->error[j] = Csub (blms->delay_line[j], blms->y[j]);	// compute error signal

      if (blms->filter_type)
		  memcpy (&CXBdata (blms->signal, sigidx), &blms->y[128], 128 * sizeof  (DTTSPCOMPLEX) );	// if noise filter, output y
      else
		  memcpy (&CXBdata (blms->signal, sigidx), &blms->error[128], 128 * sizeof  (DTTSPCOMPLEX) );	// if notch filter, output error
      ...
      memset (&blms->update[128], 0, sizeof  (DTTSPCOMPLEX)  * 128);	// zero the last block of the update, so we get
