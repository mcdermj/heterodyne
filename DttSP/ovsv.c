...
filter_OvSv(FiltOvSv pflt) {
  ...
  memcpy((void *)zrvec,(const void *)&zrvec[n],n*sizeof (DTTSPCOMPLEX) );
}

void
reset_OvSv(FiltOvSv pflt) 
{
  memset((char *) pflt->zrvec, 0, pflt->fftlen * sizeof (DTTSPCOMPLEX) );
  memset((char *) pflt->zovec, 0, pflt->fftlen * sizeof (DTTSPCOMPLEX) );
}
...
FiltOvSv
newFiltOvSv (DTTSPCOMPLEX * coefs, int ncoef, int pbits) {
  int buflen, fftlen;
  FiltOvSv p;
  fftwf_plan pfwd, pinv;
  DTTSPCOMPLEX *zrvec, *zfvec, *zivec, *zovec;
