...
{
	semaphore_wait(top[0].sync.upd.sem);
	semaphore_wait(top[1].sync.upd.sem);
	semaphore_wait(top[2].sync.upd.sem);
	if ((setit > 0) && (setit<4)) threadno = setit;
	semaphore_signal(top[2].sync.upd.sem);
	semaphore_signal(top[1].sync.upd.sem);
	semaphore_signal(top[0].sync.upd.sem);
}
...
{
	semaphore_wait(top[thread].sync.upd.sem);
	...
	semaphore_signal(top[thread].sync.upd.sem);
 ...
 DTTSPCOMPLEX *zcvec;
	...
	if (rtn == 0)
	{
		...
		memcpy ((char *) rx[thread][subrx].filt.save, (char *) rx[thread][subrx].filt.ovsv->zfvec,
			rx[thread][subrx].filt.ovsv->fftlen * sizeof  (DTTSPCOMPLEX) );
...
/*  if (n < 11)
    ...
    fftwf_plan ptmp;
    ComplexFIR tmpfilt;
    DTTSPCOMPLEX *filtcoef = newvec_COMPLEX(512, "filter for EQ"),
            *tmpcoef  = newvec_COMPLEX(257, "tmp filter for EQ");
...
Process_ComplexSpectrum (unsigned int thread, float *results)
{
	...
	memcpy ((void *) results, uni[thread].spec.coutput, uni[thread].spec.size * sizeof  (DTTSPCOMPLEX) );
}
...
DttSP_EXP void
DoResampler  (DTTSPCOMPLEX * input, DTTSPCOMPLEX * output, int numsamps, int *outsamps,
             ResSt ptr)
{
	ptr->input = input;
	ptr->output = output;
