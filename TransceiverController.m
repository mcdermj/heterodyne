...
-(void)start {
	...
	interface = [[[drivers objectAtIndex:0] alloc] initWithSDR:sdr];
	...
	[NSTimer scheduledTimerWithTimeInterval:0.06 target: self selector: @selector(updateMeter:) userInfo: nil repeats: YES];
...
-(void)setSampleRate:(int) theSampleRate {
	...
	NSLog(@"Setting DSP Sample Rate to %d\n", theSampleRate);
	// [interface setSampleRate:sampleRate];
	[self initDSP];
}
...
-(IBAction)doPreferences:(id) sender {
	...
	if(![[[prefsPane contentView] subviews] containsObject:[interface configWindow]]) {
		...
		//[[prefsPane contentView] setNeedsDisplay: YES];
